        ...
        <div class="wrapper-page">
        	...
        	<div class="m-t-40 card-box">
                <div class="text-center">
					...
					<p class="text-muted m-b-0 font-13 m-t-20">Digite a nova senha abaixo para alterá-la.</p>
                </div>

                <?php
                // Notificações
                // Impressão dos códigos de notificações
                if (isset($_SESSION['data_notificacoes']['adminRecoverPwdNew'])) {
                    ?><div class="text-center m-t-15"><?php
                    foreach ($_SESSION['data_notificacoes']['adminRecoverPwdNew'] as $item) {
                        echo $item;
                    }
                    ?></div<?php
                    unset($_SESSION['data_notificacoes']['adminRecoverPwdNew']);
                }
                ?>

                <div class="panel-body">
                    <form class="form-horizontal m-t-20" method="post" action="<?=URL_BASE?>/<?=$this->core->action_urlName?>/<?=$this->command?>/recuperar-senha-nova?token=<?=$token?>&email=<?=$email?>">

                        <div class="form-group">
                            <div class="col-xs-12">
                                <input class="form-control" type="password" name="password" required="" placeholder="Digite a nova senha">
                            </div>
                            <div class="col-xs-12 m-t-15">
                                <input class="form-control" type="password" name="repeatPassword" required="" placeholder="Digite novamente a nova senha">
                            </div>
                        </div>
