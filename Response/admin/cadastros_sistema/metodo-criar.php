<?php require PATH_ABS . '/Response/admin/_includes/php/head.php'; ?>
    ...
    <body>
        ...
        <div class="wrapper">
            <div class="container">
                <form role="form" method="post" action="<?=URL_BASE?>/<?=$this->core->action_urlName?>/<?=$this->commandArchive?>/metodo-criar/<?=$id_edicao?>">

                    <!-- Page-Title -->
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="btn-group pull-right m-t-15">
                                <button type="submit" name="submit" class="btn btn-success waves-effect waves-light pull-right">Cadastrar</button>
                            </div>

                            <h4 class="page-title">Cadastrar Novo Método</h4>
                        </div>
                    </div>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-12 col-md-6">
                                        <div class="form-horizontal" role="form">
                                            ...
                                            <div class="form-group">
                                                <label class="col-md-3 control-label" for="rotulo">Método ativado?</label>
                                                <div class="col-md-9">
                                    ...
                                    <div class="col-sm-12 col-md-6">
                                        <div class="form-horizontal" role="form">
                                            <div class="form-group">
                                                <label class="col-md-3 control-label" for="id_comando_pai">Comando correspondente *</label>
                                                <div class="col-md-9">
                                                    <select class="form-control" name="id_comando_pai" id="id_comando_pai">
                                                        <?php
                                                        // Separa os comandos entre pais e filhos
                                                        foreach ($comandos as $comando) {
                                                            // Hierarquiza os comandos
                                                            if ($comando['id_comando_pai'] == 0) {
                                                                $lista_comandos[$comando['id']] = $comando;
                                                            } else {
                                                                $lista_comandos[$comando['id_comando_pai']]['lista_comandosFilho'][] = $comando;
                                                            }
                                                        }

                                                        // Lista os comandos de forma hierárquica
                                                        // Lista os comandos-pai
                                                        foreach ($lista_comandos as $comando) {
                                                            ?>
                                                            <option 
                                                                    value="<?=$comando['id']?>"
                                                                    <?php
                                                                    if ($comando['id'] == $data['id_comando_pai']) {
                                                                    ?>
                                                                    selected
                                                                    <?php
                                                                    }
                                                                    ?>
                                                                    ><?=$comando['rotulo']?></option>
                                                            <?php

                                                            // Lista os comandos-filho
                                                            if (is_array($comando['lista_comandosFilho'])) {
                                                                foreach ($comando['lista_comandosFilho'] as $comandoFilho) {
                                                                    ?>
                                                                    <option 
                                                                            value="<?=$comandoFilho['id']?>"
                                                                            <?php
                                                                            if ($comandoFilho['id'] == $data['id_comando_pai']) {
                                                                            ?>
                                                                            selected
                                                                            <?php
                                                                            }
                                                                            ?>
                                                                            >&rdca; <?=$comandoFilho['rotulo']?></option>
                                                                    <?php
                                                                }
                                                            }
                                                        }
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-md-9">
                                                    <select class="form-control" name="local" id="local">
                                                        <option value="1" <?php if (1 == $data['local']) { ?>selected<?php } ?>>Botão superior</option>
                                                        <option value="2" <?php if (2 == $data['local']) { ?>selected<?php } ?>>Opção de linha de tabela</option>
                                                    </select>
                                                </div>
                                            </div>
                                            <div class="form-group">
