...
if (isset($_POST['submit'])) {
    // Valida o formulário
    $validate = $FirstMoldHierarchyModule->command_validateFormData(true);

    // Caso os dados estejam ok, edita
    if ($validate == true) {
        // Edita
        $updateStatus = $FirstMoldHierarchyModule->command_update($id_edicao);
...
$data = $FirstMoldHierarchyModule->command_read('WHERE id = ' . $id_edicao)[0];
